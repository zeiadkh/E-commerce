...
export const register = async (req, res, next) =>{
    ...
    return await sendEmail({to:email, subject: "Confrimation Your Email", temp: confirmationTemp(confirmationLink)})?
    
    res.json({succes: true, message: "Account Created Successfully, Please Check your email"}):
...
export const login = async(req, res, next) => {
    const user = await User.findOne({email: req.body.email, isConfirmed: true})
    ...
    await Token.create({token, user: user._id, agent: req.header["User-Agent"]})
    user.status = "online"
    await user.save()
...
export const forgetPass = async (req, res, next) => {
    ...
    await user.save()
    return sendEmail({to: user.email,subject: "reset Password",temp: resetTemp(code)})
...
export const resetPass = async (req, res, next) =>{
    ...
    tokens.forEach(async token => {token.isValid = false;await token.save()} )
    
    return res.json({success: true, message: "Password updated successfully, Your logged out from all devices"})
}